// insert data for specific element
let insertData = (cardEl, data) => {
  // remove extra loading bar from DOM
  cardEl.querySelectorAll('.loading').forEach(el => el.remove());

  // remove bg-move animation
  cardEl.querySelectorAll('.bg-move').forEach(el => el.classList.remove('bg-move'));

  // insert card img
  cardEl.querySelector('.card-header').innerHTML = `<img src=${data.img} class='h-full w-full object-cover' alt=${data.title} />`;
  ...
  // insert avatar
  cardEl.querySelector('.avatar').innerHTML = /*html*/ `<img class="h-full w-full" src=${data.avatar} alt=${data.name} />`;

  // insert name, date
  cardEl.querySelector('.profile').insertAdjacentHTML(
    'beforeend',
    `<div>
    <p class="font-[700]">${data.name}</p>
    <p class="text-[12px] text-gray-400">${data.date}</p>
  </div>`
  );
};

// blank card HTML
let blankCard = /*html*/ ` <section
  class="card flex h-[400px] w-[350px] flex-col items-center justify-center overflow-hidden rounded-xl shadow-lg"
>
  <div class="card-header bg-move h-1/2 w-full" style="padding"></div>
  <div class="card-content h-1/2 w-full bg-white px-[25px]">
    ...
    <div class="profile justify-begin mt-[20px] flex items-center">
      <div class="avatar bg-move mr-4 h-[40px] w-[40px] overflow-hidden rounded-full"></div>
      <div class="loading flex flex-col items-center justify-center space-y-2">
        <p class="bg-move h-[10px] w-[100px] rounded-full"></p>
        <p class="bg-move h-[10px] w-[100px] rounded-full"></p>
      </div>
    </div>
  </div>
